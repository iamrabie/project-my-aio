  ...
  return (
    <>
      <div className="flex flex-col gap-y-14">
        ...
        {/* nav container */}
        <div className="mx-auto border border-[#FFFFFF14] w-[fit-content] rounded-xl bg-gradient-to-r from-purple-900/75 to-blue-900/75">
          <ul className="flex py-2 px-8">
            {nav.map((item) => (
              <button className="focus:bg-[#6721FF] focus:transition focus:delay-75 rounded-xl">
                <li
                  key={item.id}
                  className="text-white font-semibold px-4 py-2.5 rounded-xl text-[15px]"
                  onClick={() => {
